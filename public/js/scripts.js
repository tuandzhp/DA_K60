  ...
  get3LegToken(function (token) {
    ...
    if (!token) {
      ...
      auth.click(function () {
        if (
          confirm(
            "You're logged in and your token is " +
              token +
...
function logoff() {
  ...
  $('#hiddenFrame').load(function (_event) {
    ...
    download.click(function () {
      ...
      if (format === 'obj') {
        ...
        if (nodeIds.length) {
          ...
          $.each(nodeIds, function (_index, value) {
...
function prepareFilesTree() {
  ...
  $('#apsFiles')
    ...
    .bind('select_node.jstree', function (_evt, data) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        data.data.forEach(function (item) {
          if (
            item.meta.extension.type === 'auxiliary:autodesk.core:Attachment'
          ) {
            var menuItem = {
              label: item.displayName,
              action: function (obj) {
                alert(
                  obj.item.label + ' with versionId = ' + obj.item.versionId,
                )
              },
              versionId: item.id,
              submenu: {
...
function prepareHierarchyTree(urn, guid, json) {
  ...
  $('#apsHierarchy')
    ...
    .bind('check_node.jstree uncheck_node.jstree', function (_evt, _data) {
...
function hierarchyTreeContextMenu(node, _callback) {
  var menuItems = {}

  var menuItem = {
    label: 'Select in Fusion',
    action: function (_obj) {
...
function preparePropertyTree(_urn, _guid, objectId, props) {
  // Convert data to expected format
  ...
  $('#apsProperties')
    ...
    .bind('activate_node.jstree', function () {
      //
    })
}
...
function showAllProperties(viewer) {
  ...
  for (var key in allDbIds) {
    var id = allDbIds[key]
    viewer.model.getProperties(id, function (_data) {})
  }
}
  ...
  button.onClick = function (_e) {
    ...
    if (ids.length === 1) {
      ...
      $.ajax({
        ...
        .fail(function (_xhr, _ajaxOptions, _thrownError) {
